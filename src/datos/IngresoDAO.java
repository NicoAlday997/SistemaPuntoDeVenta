...
public class IngresoDAO implements CrudIngresoInterface<Ingreso, DetalleIngreso>{
    ...
    public List<DetalleIngreso> listarDetalle(int id) {
		...
		try {
		      ps=CON.conectar().prepareStatement("SELECT a.id,a.codigo,a.formato,a.nombre,d.cantidad,d.piezas,d.precio,(d.cantidad*precio) as sub_total FROM detalle_ingreso d INNER JOIN articulo a ON d.articulo_id=a.id WHERE d.ingreso_id=?");
			...
			while(rs.next())
			{
				registros.add(new DetalleIngreso(rs.getInt(1), rs.getString(2), rs.getInt(3), rs.getString(4), rs.getInt(5),rs.getInt(6),rs.getDouble(7),rs.getDouble(8)));
			}
			ps.close();
			rs.close();
		} catch (SQLException e) {
    ...
    public boolean insertar(Ingreso obj) {
		...
		try { 
                    ...
                    if(filasAfectadas==1){
                        String sqlInsertDetalle="INSERT INTO detalle_ingreso (ingreso_id, articulo_id, cantidad, piezas, precio, existencia, existencia_piezas ) VALUES (?,?,?,?,?,?,?)";
                        ...
                        for(DetalleIngreso item: obj.getDetalles()){
                            ...
                            ps.setInt(7, item.getExistenciaPiezas());
    ...
    public boolean anular(int id) {
        ...
        resp=false;
		try {
                        ps=CON.conectar().prepareStatement("SELECT estado from ingreso WHERE id=?");
                        ps.setInt(1, id);
    ...
    public String ultimoSerie(String tipoComprobante) {
        ...
        try {
            ps=CON.conectar().prepareStatement("SELECT serie_comprobante FROM ingreso where tipo_comprobante=? order by cast(serie_comprobante as decimal) desc limit 1");            
    ...
    public String ultimoNumero(String tipoComprobante,String serieComprobante) {
        ...
        try {
            ps=CON.conectar().prepareStatement("SELECT num_comprobante FROM ingreso WHERE tipo_comprobante=? AND serie_comprobante=? order by cast(num_comprobante as decimal) desc limit 1;");            
            ps.setString(1, tipoComprobante);
      ...
      public List<Ingreso> consultaFechas(Date fechaInicio, Date fechaFin) {
        List<Ingreso> registros=new ArrayList();
        try {
            ps=CON.conectar().prepareStatement("SELECT i.id,i.usuario_id,u.nombre as usuario_nombre,i.persona_id,p.nombre as persona_nombre,i.tipo_comprobante,i.serie_comprobante,i.num_comprobante,i.fecha,i.impuesto,i.total,i.estado FROM ingreso i INNER JOIN persona p ON i.persona_id=p.id INNER JOIN usuario u ON i.usuario_id=u.id WHERE i.fecha>=? AND i.fecha<=? order by i.fecha asc");
